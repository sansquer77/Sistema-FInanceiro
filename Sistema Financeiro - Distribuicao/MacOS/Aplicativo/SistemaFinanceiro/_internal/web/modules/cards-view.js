export function registerCardsView({
  ...
  function invoiceHistoryRows(card) {
    const months = [-2, -1, 0, 1, 2].map((offset) => {
      const month = shiftMonth(state.cardInvoiceMonth, offset);
      return {
        offset,
        month,
    ...
    const max = Math.max(...months.map((row) => Math.abs(row.amount)), 1);
  ...
  function invoiceHistoryPath(rows, mode = "all") {
    const visibleRows = mode === "future" ? rows.slice(2) : mode === "past" ? rows.slice(0, 3) : rows;
    if (visibleRows.length < 2) {
      return "";
    }
    return smoothInvoicePath(visibleRows.map((row) => invoicePoint(row)));
  }

  function invoiceHistoryAreaPath(rows) {
    const points = rows.map((row) => invoicePoint(row));
    if (points.length < 2) {
      return "";
    }
    const line = smoothInvoicePath(points);
    const first = points[0];
    const last = points[points.length - 1];
    return `${line} L ${last.x} 100 L ${first.x} 100 Z`;
  }

  function invoicePoint(row) {
    return {
      x: row.x,
      y: row.y,
    };
  ...
  function smoothInvoicePath(points) {
  ...
  async function handleCardInvoiceHistoryClick(event) {
    const button = event.target.closest("[data-invoice-history-month]");
    if (!button) {
      return;
    }
    await setCardInvoiceMonth(button.dataset.invoiceHistoryMonth);
  }
