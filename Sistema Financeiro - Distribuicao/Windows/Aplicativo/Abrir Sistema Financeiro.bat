...
set "RUNTIME_EXE=%APP_DIR%SistemaFinanceiro\SistemaFinanceiro.exe"
set "SISTEMA_FINANCEIRO_DATA_DIR=%APP_DIR%data"

if not exist "%RUNTIME_EXE%" (
  echo Executavel do Sistema Financeiro nao encontrado.
  echo.
  echo Esperado em:
  echo %RUNTIME_EXE%
  echo.
...
if not %errorlevel%==0 (
  ...
  start "Sistema Financeiro Servidor" /D "%APP_DIR%" /min cmd /c "set APP_HOST=%APP_HOST%&& set APP_PORT=%APP_PORT%&& set APP_URL=%APP_URL%&& set APP_ALLOWED_HOSTS=%APP_ALLOWED_HOSTS%&& set APP_ALLOWED_ORIGINS=%APP_ALLOWED_ORIGINS%&& set SISTEMA_FINANCEIRO_DATA_DIR=%SISTEMA_FINANCEIRO_DATA_DIR%&& ""%RUNTIME_EXE%"" >> data\server.log 2>&1"
