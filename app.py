...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def handle_financial_health_trends(self) -> None:
        # spec: tendencias-saude-financeira v2.1 — critérios 1, 3, 4, 5, 6, 7, 13, 17, 25, 26, 27 e 28
        if not self.validate_read_source():
            return
        user = self.require_user()
        query = parse_qs(urlsplit(self.path).query)
    ...
    def handle_ai_settings_status(self) -> None:
        # spec: tendencias-saude-financeira v2.1 — critérios 17, 18 e 19
        if not self.validate_read_source():
            return
        user = self.require_user()
    ...
    def handle_save_ai_settings(self) -> None:
        # spec: tendencias-saude-financeira v2.1 — critérios 17, 18, 19, 21, 23, 27 e 28
        user = self.require_user()
        data = self.read_json()
        try:
