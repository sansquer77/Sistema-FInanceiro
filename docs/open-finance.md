...
> [!info] Status
> **rascunho** · área: `open-finance` · atualizado em 2026-08-06 · relacionados: [[contas-correntes]], [[importacao-organizze]], [[classificacao-assistida]], [[privacidade-valores]], [[adr/0005-smtp-criptografado-local]]
...
7. O usuário pode revisar as conexões ativas nas Preferências e desconectar uma conta quando quiser.
...
- Uma sincronização atualiza o saldo da conta corrente vinculada e importa transações novas em centavos, seguindo a mesma convenção monetária do restante do sistema.
- Transações já sincronizadas anteriormente (mesmo `transacao_external_id`) não geram duplicata em nova sincronização.
...
- `client_id` e `client_secret` são armazenados criptografados, seguindo o mesmo padrão de credenciais sensíveis descrito em [[adr/0005-smtp-criptografado-local]] (a decidir se por extensão desse ADR ou por um novo ADR — ver Pendências).
...
| `POST` | `/api/open-finance/sync` — botão do cabeçalho: dispara sincronização manual de saldo e transações de todas as conexões ativas do usuário autenticado. |
...
Tabela afetada: `checking_accounts` (vínculo por `checking_account_id`), `transactions` (novos lançamentos sincronizados).
...
- Dado uma conta conectada com sucesso pelo Conector 200 e vinculada a uma conta corrente, quando a sincronização roda, então saldo e transações novas são gravados nessa conta em centavos.
- Dado uma transação já sincronizada anteriormente, quando a sincronização roda novamente, então essa transação não é duplicada.
...
- [ ] Definir regra de deduplicação entre transações sincronizadas via Open Finance e transações já lançadas manualmente ou importadas via Organizze no mesmo período. Ver [[importacao-organizze]].
- [ ] Confirmar se o armazenamento criptografado de `client_id`/`client_secret` estende `financeiro/secure_config.py` (ADR-0005) ou exige um novo ADR dedicado.
...
- [ ] Definir se categorias/subcategorias das transações sincronizadas passam pela classificação assistida local ou chegam sem categoria, exigindo classificação manual. Ver [[classificacao-assistida]].
...
- [ ] Passo 1 — Resolver as Pendências de escopo e arquitetura acima, incluindo atualização de `docs/requisitos.md` e decisão sobre novo ADR ou extensão do ADR-0005 para armazenamento de credenciais. Fecha: critérios 1, 8.
- [ ] Passo 2 — Migração idempotente em `financeiro/database.py` criando `open_finance_connections` e `open_finance_sync_log`. Fecha: critérios 3, 4, 5, 7.
- [ ] Passo 3 — Novo módulo `financeiro/open_finance.py` com autenticação na API da Pluggy, geração de connect token, sincronização de contas/transações e desconexão. Fecha: critérios 1, 2, 3, 4, 6.
- [ ] Passo 4 — Rotas em `app.py` expondo os endpoints da tabela acima, validando sessão e propriedade da conexão, incluindo o parâmetro `vincular_open_finance` em `POST /api/checking-accounts`. Fecha: critérios 1, 2, 5, 7, 8, 9, 10.
- [ ] Passo 5 — View no frontend (`web/modules/`) para configurar credenciais por usuário, iniciar o widget, listar conexões, desconectar, o botão de sincronização manual no cabeçalho (ao lado do controle de [[privacidade-valores]]) e a opção `vincular_open_finance` no formulário de criação de conta. Fecha: critérios 1, 7, 8, 9, 10, 11.
- [ ] Passo 6 — Testes automatizados cobrindo os critérios acima, com mocks da API da Pluggy (sem chamadas reais em teste).
