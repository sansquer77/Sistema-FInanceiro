...
> [!info] Status
> **implementado** · área: `arquitetura-v2` · atualizado em 2026-08-30 · relacionados: [[../arquitetura]], [[../adr/0014-desconcentracao-fachadas-e-roteamento]], [[investimentos-portfolio]], [[frontend-modularizacao]]
...
- Rotas existentes preservadas; nenhuma rota nova nesta entrega.
...
- `transactions.py` e `credit_cards.py`: extrair projeções compartilhadas somente para um serviço Python de saldos/faturas; não criar uma nova implementação financeira em JavaScript.
- `web/app.js`: os wrappers de views sem função contratual foram removidos. O bloco legado de projeção financeira foi identificado como dívida de fronteira e deve sair apenas junto do contrato backend correspondente, evitando uma mera mudança para outro módulo JS.
...
- [x] Passo 4 — auditar concentração e registrar candidatos seguintes. Fecha: critério 7.
