...
> [!info] Status
> **rascunho** · área: `landing-page` · atualizado em 2026-08-02 · relacionados: [[sobre-app]], [[../design/design-system|Design System]], [[../adr/0007-landing-page-institucional-isolada|ADR-0007]]
...
- `landing-page/assets/trends-preview.png`: captura de tela demonstrativa da aba Tendências do Cockpit.
...
- `landing-page/assets/pix-qrcode.png`: QR Code PIX informado pelo mantenedor para pagamento.
- `landing-page/assets/demo-data/`: dados demonstrativos ou fixtures visuais usados para gerar capturas fictícias, sem relação com a base de homologação.

### Regras

- A Landing Page é um site/apresentação institucional isolado no diretório `landing-page/`, inspirado em templates do **v0.app**, especialmente no modelo visual “COMPUTE - The Platform to Build & Ship AI Agents”.
- A Landing Page pode usar stack própria de site institucional, incluindo Next.js, React, Tailwind e dependências npm, desde que permaneça confinada ao diretório `landing-page/` e seja tratada como subprojeto independente do app principal.
...
- O deploy recomendado da landing page é via Vercel apontando o **Root Directory** para `landing-page/`.
- O código exportado do v0.app deve ser descompactado diretamente dentro de `landing-page/`, evitando criar um nível intermediário como `landing-page/compute-the-platform.../`.
- Ao descompactar o template em `landing-page/`, os assets demonstrativos já gerados em `landing-page/assets/` devem ser preservados ou copiados para `landing-page/public/` conforme a estrutura final do projeto Next.
...
- Todas as imagens demonstrativas em `landing-page/assets/` devem utilizar dados fictícios, com nomes, valores, contas, cartões, investimentos, saldos, limites e e-mails diferentes dos dados reais e da base de homologação.
...
- Conteúdo institucional armazenado em `landing-page/`, com dependências, scripts e configuração próprios quando a landing usar Next.js/React.
...
- Dado a pasta `landing-page/`, quando um desenvolvedor roda o build do subprojeto ou a Vercel publica esse diretório, então os recursos são carregados sem dependência da API backend em execução.
- Dado o app principal sendo distribuído para usuários finais, quando o pacote é gerado, então `landing-page/` não é incluído no pacote.

### Pendências

- [ ] Definir quais elementos visuais do template “COMPUTE - The Platform to Build & Ship AI Agents” serão adaptados, especialmente a árvore/narrativa modular.
- [ ] Definir texto final, valor, chave/QR Code PIX e canal de contato para solicitação de cópia.
- [ ] Gerar e salvar as imagens de captura de tela em `landing-page/assets/` utilizando massa fictícia independente da base de homologação.
- [x] Definir se a landing page será hospedada via Vercel/GitHub Pages ou servida opcionalmente no `app.py`: decisão atual é Vercel com root em `landing-page/`, sem servir pelo `app.py`.
...
- Inclusão da landing page nos pacotes instaláveis macOS/Windows/Linux do Sistema Financeiro.
- Uso da stack Next/React/Tailwind da landing como precedente para alterar a stack do app principal.

### Plano de implementação

- [ ] Passo 1 — Definir a narrativa visual da árvore, copy principal e CTA PIX/manual, sem alterar código do app principal. Fecha: critérios 1, 2 e 6.
- [ ] Passo 2 — Criar o diretório `landing-page/` e a pasta `landing-page/assets/` com QR Code PIX e capturas/mockups fictícios dos módulos. Fecha: critérios 4, 5 e 8.
- [ ] Passo 3 — Descompactar/adaptar o projeto exportado do v0.app diretamente em `landing-page/`, preservando os assets demonstrativos e configurando a Vercel para usar esse diretório como raiz. Fecha: critérios 1, 2, 3, 4, 6, 7, 9 e 10.
