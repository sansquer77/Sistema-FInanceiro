...
def list_transactions(
    ...
    if month:
        ...
        if account_id:
            # spec: lancamentos v3.34 — a fatia de mes+conta mantem todo o historico
...
def update_transaction(user_id: int, transaction_id: str, data: dict) -> dict:
    ...
    with get_connection() as conn:
        ...
        # spec: lancamentos v3.34 — criterios 46, 56, 57 e 60
...
def update_future_series_transactions(conn, user_id: int, existing, transaction: dict, force_apply_to_future: bool = False) -> None:
    # spec: lancamentos v3.34 — criterios 56, 57, 59 e 60
    ...
    # spec: lancamentos v3.34 — criterio 7 (apply_to_future)
    ...
    category_id, subcategory_id = resolve_transaction_category(conn, user_id, transaction, destination)
    # spec: lancamentos v3.34 — criterios 56, 57 e 59
...
def insert_additional_series_occurrences(
    ...
    # spec: lancamentos v3.34 — critério 47
...
def future_transactions_to_delete(conn, user_id: int, transaction, apply_to_future: bool):
    # spec: lancamentos v3.34 — criterio 6 (scope=future)
...
def normalize_series_payload(data: dict) -> dict:
    ...
    if series_kind == "recurring":
        ...
        # spec: lancamentos v3.34 — critérios 24 e 25
...
def normalize_investment_emergency_reserve_eligible(data: dict, asset_type: str) -> int:
    # spec: investimentos/investimentos-portfolio v2.51 — critérios 21 e 23
...
def average_amount_for_recurring_description(
    ...
    # spec: lancamentos v3.34 — critério 27
...
def build_transaction_occurrences(transaction: dict) -> list[dict]:
    # spec: lancamentos v3.34 — regra de parcelamento/recorrencia (secao "Regras de negocio")
